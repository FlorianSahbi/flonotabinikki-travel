// @path: src/features/experience/ExperienceView.tsx
'use client'
...
export default function ExperienceView({ data }: { data: VideoExperience }) {
  const sources = data.videos.map((v) => v.main_url).filter(Boolean) as string[]
  const dateRangeText =
    data.dateRangeLabels.length > 0 ? data.dateRangeLabels.join(' — ') : ''
    ...
    <div className="relative">
      ...
      <div className="relative z-10 flex h-screen flex-col items-center justify-center px-8 text-center text-white">
        ...
        >
          ...
          <div className="text-sm text-white/70">{dateRangeText}</div>
        </motion.div>
